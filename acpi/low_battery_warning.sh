...
nag() {
  ...
  DISPLAY=:0
  if [ -e /proc/${NAG_PID} -a /proc/${NAG_PID}/exe -ef $NAG_EXEC ]; then
...
if [ $MINS_LEFT -gt THRESHOLD_SHOW_NAG ]; then
  exit 0
fi

if [ $MINS_LEFT -gt THRESHOLD_HIBERNATE ]; then
  nag
  exit 0
else
